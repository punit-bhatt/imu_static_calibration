...
def calibrate_imu():
    ...
    a3_imu_topic = rospy.get_param('~a3_imu_topic')
    ouster_imu_topic = rospy.get_param('~ouster_imu_topic')
    ...
    a3_linear_acc, a3_angular_vel = get_norm_imu_readings(bag_path,
                                                          a3_imu_topic)
    ouster_linear_acc, ouster_angular_vel = \
        get_norm_imu_readings(bag_path, ouster_imu_topic)

    print(f'{a3_linear_acc.shape = }')
    print(f'{ouster_linear_acc.shape = }')

    omega = get_axis_of_rotation(a3_linear_acc, ouster_linear_acc)
    theta = get_3d_rotation_angle(a3_linear_acc, ouster_linear_acc, omega)
    R = get_rotation_matrix(omega, theta)

    print(f'\n{omega = }')
    print(f'{theta = }')
    print(f'{R = }')

    # verify_rotation_matrix(a3_linear_acc, ouster_linear_acc, R)
    write_matrix_to_file(out_path, R, a3_linear_acc, ouster_linear_acc)
